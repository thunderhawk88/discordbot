...
Function Get-ItemDetails {
    ...
    # Patterns
    [string]$private:ImagePattern = '(?<= src=").+?(?=">)'                               # get all strings between " src="" and "">"
    [string]$private:LinkPattern1  = '(?<=<H3><A title=").+?(?=" rel=)'                  # get all strings between "<H3><A title="" and "" rel="
    [string]$private:LinkPattern2  = '(?<=<H3><A title=).+?(?=" rel=)'                   # get all strings between "<H3><A title=" and "" rel="
...
# Website url
$URL = "https://mangakakalot.com/manga_list?type=topview&category=all&state=all&page=$TargetPage"
...
if (-not($HTMLPage)) {
    if ($DebugMode) {Write-Host "Error: $($LastError)"}
    exit 503
}
